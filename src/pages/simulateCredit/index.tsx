...
import { stepsSimulateCredit } from "./config/addProspect.config";
...
export function SimulateCredit() {
  const [currentStep, setCurrentStep] = useState<number>(
    stepsSimulateCredit.generalInformation.id,
  );
  ...
  const steps = Object.values(stepsSimulateCredit);
  ...
  useEffect(() => {
    if (currentStep === stepsSimulateCredit.productSelection.id) {
      setFormData((prevState) => ({
        ...prevState,
        togglesState: [false, false, false, false],
      }));
    }
    if (currentStep === stepsSimulateCredit.destination.id) {
      setFormData((prevState) => ({
        ...prevState,
        selectedProducts: [],
      }));
  ...
  const handleNextStep = () => {
    ...
    const isInExtraBorrowersStep =
      currentStep === stepsSimulateCredit.extraBorrowers.id;
    ...
    const dynamicSteps = [
      togglesState[0]
        ? stepsSimulateCredit.extraordinaryInstallments.id
        : undefined,
      togglesState[3] ? stepsSimulateCredit.extraBorrowers.id : undefined,
      ...[
        !(isInExtraBorrowersStep && totalIncome !== 0) && showSourcesIncome
          ? stepsSimulateCredit.sourcesIncome.id
          : undefined,
        !(isInExtraBorrowersStep && totalObligations !== undefined) &&
        showObligations
          ? stepsSimulateCredit.obligationsFinancial.id
          : undefined,
      ],
      stepsSimulateCredit.loanConditions.id,
      stepsSimulateCredit.loanAmount.id,
      loanAmountState.toggleChecked
        ? stepsSimulateCredit.obligationsCollected.id
        : undefined,
    ...
    if (
      currentStep === stepsSimulateCredit.loanAmount.id &&
      !loanAmountState.toggleChecked
    ) {
      handleSubmitClick();
      return;
    }
    if (currentStep === stepsSimulateCredit.loanConditions.id) {
      showConsultingForFiveSeconds();
    }
    if (
      currentStep === stepsSimulateCredit.sourcesIncome.id &&
      totalIncome === 0
    ) {
      setIsAlertIncome(true);
      return;
    }
    if (currentStep === stepsSimulateCredit.sourcesIncome.id) {
      setCurrentStep(stepsSimulateCredit.obligationsFinancial.id);
      return;
    }
    if (
      currentStep === stepsSimulateCredit.obligationsFinancial.id &&
  ...
  const handlePreviousStep = () => {
    ...
    const nextStepWouldBeExtraBorrowers =
      currentStep === stepsSimulateCredit.loanConditions.id && togglesState[3];
    ...
    if (currentStepIndex > 0) {
      ...
      setCurrentStep(stepsSimulateCredit.productSelection.id);
