...
export const mockData: IEntries[] = [
  {
    id: "1",
    "Validaciones del sistema": "Que el asociado sea activo y tiene mas de 5 años de antiguedad",
    tag: (
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <Icon
          ...
          appearance={appearanceIcon(
            isValidElement(data?.tag) ? data?.tag?.props?.children?.props?.label : "primary",
          )}
  ...
  },
  {
    ...
    content: (data: IEntries) => (
      <Icon
        ...
        onClick={() => receiveData(data)}
        disabled={
          isValidElement(data?.tag) && data?.tag?.props?.children?.props?.label === "Sin Validar"
        }
      />
    ),
  },
...
export const actionMobileMock: IAction[] = [
  {
    ...
    content: (data) => (
      <Icon
        ...
        appearance={isValidTagElement(data?.tag) && data?.tag?.props?.children?.props?.appearance}
        spacing="compact"
        cursorHover
        variant="filled"
        shape="circle"
